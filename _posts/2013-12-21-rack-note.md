...
* rackup 实现
  ...
  * `self.middleware` rackup 根据不同的环境（可以用-E开关选择环境)装载不同的中间件
  ...
  * `options`
     ...
     `parse_options` 是的私有实例方法：
        ...
        => {:environment=>"development", :pid=>nil, :Port=>9292, :Host=>"0.0.0.0", :AccessLog=>[], :config=>"/var/www/sites/testusers/ued/tao800_fire/config.ru", :server=>"Thin"}
