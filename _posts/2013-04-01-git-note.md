...
* `git checkout -b 新分支名 [start-point]` 基于当前分支创建新分支，创建好以后自动切换到新分支上, start-point 默认是HEAD指向的提交, config 下不会有追踪分支记录
...
* `git remote update` 获取所有的的远程版本库的分支更新, 可选参数 -p (prune remotes after fetching)???
...
* `git fetch [<options>] [<repository> [<refspec>...]]` 将从指定的远程版本库获取指定的分支信息，放到remote/远程版本库名/ repository 默认是origin，如果没有指定 refspec, 将获取所有的的远程分支。
...
`git push [<remote 远程版本库名> [refspec 要推送到的远程分支]]` 
  ...
  第一次push本地创建的分支后， 远程分支的 git/refs/heads/下会出现该分支，但是该分支在配置中还是没有追踪配置
...
`git pull [<remote 远程版本库名> [refspec 要推送到的远程分支]]` 
