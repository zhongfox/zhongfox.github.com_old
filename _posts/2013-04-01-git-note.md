...
`[remote "origin"]` : 定义了一个名为origin的远程版本库，一个版本库最多只有一个远程版本库。

`[remote "origin"].url` 远程版本库的地址，也就是clone时的地址 

`[remote "origin"].fetch=+refs/heads/*:refs/remotes/origin/*` : 定义fetch时获取的远程分支映射
