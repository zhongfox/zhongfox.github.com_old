...
* `git branch --merged [分支名]`  列出所有已经合并到"分支名"(默认当前分支)的(本地/远程-r/所有-a)分支
  ...
  `git branch -r --merged remotes/ued_assets/some_branch`  检测远程分支中, 哪些合并到了`remotes/ued_assets/some_branch`. 做这个之前最好`git remote update -p ued_assets` 其中ued_assets代码一个remote
