...
* table
  ...
  对table键值对的读写类似javascript: 可以用`table_object[key_name]`也可以用`table_object.key_name`
...
* 逻辑运算
  ...
  三元运算: `x and y or z` 等价于 `x ? y : z` (貌似也没啥用)
...
* local variables have their scope limited to the block where they are declared. A block is the body of a control structure, the body of a function, or a chunk (the file or string where the variable is declared)
* In interactive mode, each line is a chunk by itself (unless it is not a complete command) 可以使用`do end`显式限定一个block
...
* return 必须是block的最后一句, 如果要非要在中间return, 需要用do end包起来: `do return end` (do end 可以单独使用, 以定义一个代码块) (没理解)
* 调用时候的括号:
  * 如果调用时无参数, 必须有括号
...
* You can call a function with a number of arguments different from its number of parameters, 类似多重赋值: extra arguments are thrown away; extra parameters get nil
...
* When a function call is the last (or the only) argument to another call, all results from the first call go as arguments
  如果最为另一个函数的参数的函数调用不是最后一个参数的话, 此函数返回值将只有第一个
* 如果函数调用是在表达式中, Lua adjusts the number of results to one
* A constructor also collects all results from a call, without any adjustments: `t={function_return_a_table()}`
...
* `table.unpack` receives an array and returns as results all elements from the array, starting from index 1
  it uses the length operator to know how many elements to return, 但你也可以手动指定需要的范围: `print(table.unpack({10,20,30,40}, 2, 3)) => 20, 30` 左开右开
* 可变参数函数: `function name(...)` 三个点代表可变参数(书上叫做extra arguments)
  ...
  `{...}`返回一个table, 但是去掉了其中是nil的值(如果传入参数有nil的话)
